...
export default class DictSelect extends Vue {
  ...
  public async getList () {
    ...
    await getList({
      ...
      if (res.object) {
        ...
        if (this.list.length === 1 && this.selectOne) {
          this.valSync = this.list[0][this.prop.key]
          this.$listeners.change && this.$emit('change', this.list[0][this.prop.key], this.list[0][this.prop.value])
        }
      }
    }).catch(err => {
      console.log('err', err)
  ...
  private async getDatas () {
    ...
    this.loading = this.hasLoading && !0
    if (this.data) {
      this.list = this.data
    }
