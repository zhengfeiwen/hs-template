<template>
  <div class="img-verify" style="height:48px;width:120px; text-align:center;">
...
export default class VueImgVerify extends Vue {
  ...
  // 点击图片重新绘制
  private handleDraw () {
    this.$emit('get-img-code', this.draw())
  }

  // 1.随机数
  private randomNum (min: any, max: any) {
    return parseInt(Math.random() * (max - min) + min)
  }

  // 2.随机颜色
  private randomColor (min: any, max: any) {
  ...
  private draw () {
    // 3.填充背景颜色，背景颜色要浅一点
    const ctx = (this.$refs.verify as any).getContext('2d')
    // 填充颜色
