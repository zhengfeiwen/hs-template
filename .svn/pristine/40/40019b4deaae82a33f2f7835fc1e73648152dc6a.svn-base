<template>
  ...
  >
    <router-link
      ref="tag"
      v-if="visitedViews && visitedViews.length > 0"
      :key="visitedViews[0].path"
      class="home-tag"
      :to="{path: visitedViews[0].path, query: visitedViews[0].query, fullPath: visitedViews[0].fullPath}"
      tag="span"
      @click.middle.native="!isAffix(visitedViews[0])?closeSelectedTag(visitedViews[0]):''"
      @click.native="itemClick(visitedViews[0])"
      @contextmenu.prevent.native="openMenu(visitedViews[0], $event)"
    >
      {{ visitedViews[0].meta.titleZh || $t('route.' + visitedViews[0].meta.title) }}
      <span
        v-if="!isAffix(visitedViews[0])"
        class="el-icon-close"
        @click.prevent.stop="closeSelectedTag(visitedViews[0])"
      />
    </router-link>
    <scroll-pane
      ref="scrollPane"
    ...
    >
      <router-link
        ...
        ref="tag"
        :key="tag.path"
        :class="tag.meta.titleZh === curName ? 'active' : ''"
        :to="{path: tag.path, query: tag.query, fullPath: tag.fullPath}"
        tag="span"
        class="tags-view-item"
        @click.middle.native="!isAffix(tag)?closeSelectedTag(tag):''"
        @click.native="itemClick(tag)"
        @contextmenu.prevent.native="openMenu(tag, $event)"
      >
        {{ tag.meta.titleZh || $t('route.' + tag.meta.title) }}
        <span
          v-if="!isAffix(tag)"
          class="el-icon-close"
...
export default class extends Vue {
  ...
  private curName = LocalStorage.getObj('menu-name')

  @Watch('$route')
  private onRouteChange () {
    this.addTags()
    this.curName = LocalStorage.getObj('menu-name')
  ...
  private isActive (route: ITagView) {
    return route.meta.titleZh === LocalStorage.getObj('menu-name') ? 'active' : ''
  }

  private itemClick (info: any) {
    MenuModule.updateMenuName(info.meta.parent)
  }
  ...
  private filterAffixTags (routes: RouteConfig[], basePath = '/', parent: any = '') {
    let tags: ITagView[] = []
    routes.forEach(route => {
      if (route.meta && route.meta.affix) {
        ...
        tags.push({
          ...
          path: tagPath,
          name: route.name,
          meta: { ...route.meta, parent: parent }
        })
      }
      if (route.children) {
        const childTags = this.filterAffixTags(route.children, route.path, route.name)
  ...
  private addTags () {
    ...
    if (name && !this.$route.meta.hidden) {
      LocalStorage.setObj('menu-name', this.$route.meta.titleZh || this.$t('route.' + this.$route.meta.title))
