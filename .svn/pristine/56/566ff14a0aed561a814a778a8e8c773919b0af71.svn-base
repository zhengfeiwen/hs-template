...
import { getToken, removeToken, getCookie, setCookie, removeCookie } from '@/utils/cookies'
...
class User extends VuexModule implements IUserState {
  ...
  private initData (res: any) {
    ...
    const expires = new Date(new Date().getTime() + hours)
    if (process.env.NODE_ENV === 'development') {
      ...
      Cookies.set('token', res.token, { expires: expires, path: '/', domain: currentDomain })
    }
  ...
  @Action
  public async Logout () {
    console.log(document.cookie, '|token:' + Cookies.get('token'), 'Logout-清空前')
    this.SET_UUID('')
    if (process.env.NODE_ENV === 'development') {
      removeCookie('token')
    } else {
    ...
    removeCookie('right_token')
    removeCookie('sidebar_status')
    removeCookie('user_id')
    removeCookie('user_id_token')
    removeCookie('uuid')
    removeCookie('menu_token')
    console.log(document.cookie, '|token:' + Cookies.get('token'), 'Logout-清空token完成')
    resetRouter()
