...
service.interceptors.response.use(
  (response :any) => {
    ...
    if (response.data && Object.keys(response.data).includes('singleSignOn')) return response
    if (res.code === -9990) {
    ...
    if (res.code !== 10000 && res.code !== -10000 && res.code !== -9998 && response.headers['content-type'].indexOf('application/json') > -1 && !res.fileInfos) {
      Message({
        showClose: !0, 
      ...
      if (!isWhite((response.config as any).url) && response.data && dataFormat && dataFormat.length > 0) {
        dataFormat.map(async (v: any) => {
          if (!v.range || v.range === 'response') {
            if (v.format.indexOf('dict') > -1) {
              for(const key in DICT_MONEY_KEY) {
                await getConstantByGroup(key)
              }
            }
          }
        })
