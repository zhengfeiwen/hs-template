import { isType, urlParamsParse, spectialTrim, isEmpty } from './common'
...
// 金额格式化
export const objFormatMoney = (obj: any, option: any) => {
  if (Object.keys(obj).length === 0) return obj
  const _obj = typeFormat(obj)
...
// 空值化
export const objFormatTrim = (obj: any, option: any = { stringNull: '-1', numberNull: -1 }) => {
  if (Object.keys(obj).length === 0) return obj
  const _obj = typeFormat(obj)
  Object.keys(_obj).map((key: any) => {
    // money
...
// 字典格式化
export const objFormatDict = (obj: any, option: any) => {
  if (Object.keys(obj).length === 0) return obj
  let _obj = typeFormat(obj)
  Object.keys(_obj).map(async (key: any) => {
    // dict
    _obj = await dictStandardization(key, _obj, option)
  })
  return isType(obj, 'string') ? qs.stringify(_obj) : _obj
}
