<template>
  <div>
   <el-dialog append-to-body :close-on-click-modal="!1" title="票据打印" :close-on-press-escape="false"
     :visible.sync="receiptVisible" :before-close="handleClose" width="350px"  id="receipt-page" @open="open">
     <div v-if="printErrorVisible" class="print-error">Web打印服务CLodop未安装启动，点击这里<a href="http://tcsrcz.oss-cn-hangzhou.aliyuncs.com/fcs/template/CLodop.zip" type="primary">下载执行安装</a>（若此前已安装过，可点<a href="http://tcsrcz.oss-cn-hangzhou.aliyuncs.com/fcs/template/CLodop.zip" type="primary">这里直接再次下载安装</a>），成功后请刷新本页面。</div>
       <div class="content-wrapper-dialog">
            <div style="text-align: center; font-size: 12pt;line-height:1.5;">{{receiptInfo.headline}}</div>
            <div style="padding:10px 0; text-align: center; font-size: 9pt;">{{receiptInfo.subtitle}}</div>
            <span class="item">流水号：<span>{{receiptInfo.billNo}}</span></span>
...
export default class PrintReceipt extends Vue {
  @Prop({ type: Number })
  private collectFeesCode!: number
  ...
  private dayjs = dayjs
  private printErrorVisible = false
  ...
  private printContent (headline:string, printSubtitle:string, address:string, lodop:any) {
    let top = 10 // 高度计算
    ...
    // 学校名称
    if (headline.length > 13) { // 需要换行
      const line1st = headline.substring(0, 13)
      const line2nd = headline.substring(13, headline.length)
      let headlineLeft = 120 - line1st.length * 7.8
      lodop.ADD_PRINT_TEXT(top, headlineLeft, '60mm', 20, line1st)
      lodop.SET_PRINT_STYLEA(0, 'FontSize', 11.5)
      top += 20
      headlineLeft = 120 - line2nd.length * 7.8
      lodop.ADD_PRINT_TEXT(top, headlineLeft, '60mm', 20, line2nd)
      lodop.SET_PRINT_STYLEA(0, 'FontSize', 11.5)
    } else {
      const headlineLeft = 120 - headline.length * 7.8
      lodop.ADD_PRINT_TEXT(top, headlineLeft, '60mm', 20, headline)
      lodop.SET_PRINT_STYLEA(0, 'FontSize', 11.5)
    }
    ...
    // 流水号 日期 姓名 年级
    top += 20
    lodop.ADD_PRINT_TEXT(top, 2, 431, 20, '流水号:' + billNo)
    ...
    this.receiptList.forEach((item:any, index:any) => {
      ...
      lodop.ADD_PRINT_TEXT(initialHeight, 160, 430, 20, item.amount.toFixed(2))
  ...
  private printNote () {
    this.loading = true
    const { address, headline, subtitle } = this.receiptInfo
    this.$nextTick(() => {
      ...
      try {
        ...
        this.printContent(headline, subtitle, address, lodop)
        ...
        try {
          ...
          if (_reslut) {
            this.loading = false
            setTimeout(() => { this.$message({ type: 'success', message: '打印完成!', showClose: !0, duration: 1500 }) }, 100)
          } else {
            this.loading = false
            setTimeout(() => {
              this.$message({
          ...
          this.loading = false
          setTimeout(() => {
            this.$message({ type: 'warning', message: '打印出错!', showClose: !0, duration: 1500 })
          }, 100)
        }
      } catch (err) {
        console.log(err)
        this.printErrorVisible = true
      }
    })
  }
  ...
  // 打印
  private async print () {
    const res = await commonApi('feesPrintIncr', { id: this.printIncr })
    console.log('res：', res)
    this.printNote()
    this.loading = false
    this.$emit('callback')
